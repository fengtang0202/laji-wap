...
<div class='page'>
   <div v-for='i in cityData' >
       ...
       <ul class='country'>
         <li  v-for='li in i.items' @click='selectItem(li.value)'>
            <p class='p1'>{{li.name}}</p>
            <p class='p2'>{{li.value|add}}</p>
         </li>
       </ul>
   </div>
       <ul class='md'>
...
export default{
    ...
    components: {
    //    cubeIndexList:IndexList
    },
    methods: {
      selectItem(value) {
          if(this.type==1){
              this.$router.push({path:'/Register',query:{value:value}})
          }else{
              this.$router.push({path:'/password',query:{value:value}})
          }
      }
    },
    mounted () {
      this.$nextTick(()=>{
        document.querySelector('.page').style.height=90+'vh'
      })
    }
}
</script>
